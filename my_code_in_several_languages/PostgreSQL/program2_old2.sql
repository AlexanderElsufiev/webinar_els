...
(select  marshr,st1 as kst,rst1 as rst from sts1 union all select  marshr,st2,rst2  from sts1)  as a
		where rst is null),
...
mar_n as
(select a.marshr,b.nom,1 as k
	from marshr as a,marshr as b where a.marshr=b.marshr and a.nom=b.nom-1 and a.reg!=b.reg),
...
(select marshr,rst,kst,reg,perelom,nom,k,
 ...
 from
(select a.marshr,rst,kst,reg,perelom,a.nom,case when k=1 or a.nom=1 or perelom='1' then 1 else 0 end as k
from marshr as a left join mar_n as b on a.marshr=b.marshr and a.nom=b.nom
) as c),
 

 sts_new as --список всех ВАРИАНТОВ  по перегонам-регионам НОВЫХ маршрутов
(select a.marshr,st1,st2,rasst,regg,b.rst as rst1,c.rst as rst2,rasst-abs(b.rst-c.rst) as dd,
 b.nom as nom1,c.nom as nom2,b.nomk as nomk1,c.nomk as nomk2,b.reg,mid,nom_sts
 from sts1 as a
...
(select nom_stsm,kst1,kst2,rast,dorlin,r1,r2,reg1,reg2,nreg1,nreg2,nom1,nom2 from
(select nom_stsm,kst1,kst2,rast,dorlin,r1,r2,reg1,reg2,nreg1,nreg2,nom1,nom2,
 ROW_NUMBER() over (partition by nom_stsm order by dr,dorlin) as nom
 from
(select nom_stsm,kst1,kst2,rast,b.dorlin,b.rst as r1,c.rst as r2,b.reglin as reg1,c.reglin as reg2,
...
stsm_2 as --кого нашли в 2 соседних линиях, точное значение расстояний
(select * from 
(select nom_stsm,kst1,kst2,rast,k,dorlin,r0,reg0,rst,st2,reg2,nreg0,nreg2,nom0,nom2,
 sum(abs(r0-rst)) over (partition by nom_stsm,st2) as sr,sum(k) over (partition by nom_stsm,st2) as sk
 from 
(select nom_stsm,kst1,kst2,rast,k,c.dorlin,r0,reg0,nreg0,nom0,d.rst,d.kst as st2,d.reglin as reg2,d.nreg as nreg2,nom as nom2 from 
(select nom_stsm,kst1,kst2,rast,k,dorlin,rst as r0,reglin as reg0,nreg as nreg0,nom as nom0 from
(select nom_stsm,kst1,kst2,kst,rast,k from   
(select nom_stsm,kst1,kst2,kst1 as kst,rast,1 as k from stsm
union all
select nom_stsm,kst1,kst2,kst2 as kst,rast,2 as k from stsm) as aa 
	where nom_stsm not in(select nom_stsm from stsm_1)) as a
	join lin as b on a.kst=b.kst) 
	as c,lin as d where c.dorlin=d.dorlin)
	as e) as f where sk=3 and sr=rast),
...
from  (select * from stsm
 where nom_stsm not in(select nom_stsm from stsm_1) and nom_stsm not in(select nom_stsm from stsm_2)) as a
...
(select  kst1,kst2,rast,nom_stsm,kst1 as st1,kst_1 as st2,dorlin1 as dorlin,nom1,nom_1 as nom2,
 rst1 as r1,rst_1 as r2,1 as np
from stsm_3
union all
select  kst1,kst2,rast,nom_stsm,  kst_1 as st1,kst_2 as st2,dorlinp as dorlin,nomp1 as nom1,nomp2 as nom2,
 rstp1 as r1,rstp2 as r2,2 as np
from stsm_3
union all 
select  kst1,kst2,rast,nom_stsm,kst_2 as st1,kst2 as st2,dorlin2 as dorlin,nom_2 as nom1,nom2,
...
(select *,
 abs(rst1-rst_1)+abs(rstp1-rstp_1)+abs(rstp2-rstp_2)+abs(rst2-rst_2) as rrr
 from
(select
...
from  (select * from stsm
 where nom_stsm not in(select nom_stsm from stsm_1) and nom_stsm not in(select nom_stsm from stsm_2)
	   and nom_stsm not in(select nom_stsm from stsm_3)) as a
join lin as b on kst1=b.kst join lin as c on kst2=c.kst
join lin2 as d on b.dorlin=d.dorlin 
...
from stsm_4
union all 
select  kst1,kst2,rast,nom_stsm,  kst_1 as st1,kstp as st2,dorlinp1 as dorlin,nomp1 as nom1,nomp_1 as nom2,
 rstp1 as r1,rstp_1 as r2,2 as np
from stsm_4
union all  
select  kst1,kst2,rast,nom_stsm,  kstp as st1,kst_2 as st2,dorlinp2 as dorlin,nomp_2 as nom1,nomp2 as nom2,
...
stsm_p  as --объединение всех найденных
(select nom_stsm,kst1,kst2,rast,dorlin,st_1,st_2,np,r1,r2,nom1,nom2,
 ...
 sum(abs(r1-r2)) over (partition by nom_stsm) as srst,sum(abs(r1-r2)) over (partition by nom_stsm order by np) as srst_
 from 
(select nom_stsm,kst1,kst2,rast,dorlin,r1,r2,reg1,reg2,kst1 as st_1,kst2 as st_2,nreg1,nreg2,nom1,nom2,0 as np from stsm_1
 union all
 
 select nom_stsm,kst1,kst2,rast,dorlin,r0 as r1,rst as r2,reg0 as reg1,reg2,kst1 as st_1,st2 as st_2,
 nreg0 as nreg1,nreg2,nom0 as nom1,nom2,1 as np 
 from stsm_2 where k=1
 union all
 select nom_stsm,kst1,kst2,rast,dorlin,rst as r1,r0 as r2,reg2 as reg1,reg0 as reg2,st2 as st_1,kst2 as st_2,
 nreg2 as nreg1,nreg0 as nreg2,nom2 as nom1,nom0 as nom2, 2 as np 
 from stsm_2 where k=2

 union all
 select  nom_stsm,kst1,kst2,rast,dorlin,r1,r2,0 as reg1,0 as reg2,st1 as st_1,st2 as st_2,
 0 as nreg1,0 as nreg2,nom1,nom2, np 
 from stsm_3_1
...
select rez,nom_bil,nom_mar,YYYYMM,date_zap,id,doc_num,TERM_POS,TERM_DOR,date,agent,chp,stp,stp_reg,
	sto,stn,sto_reg,stn_reg,srasst,k_pas,srok_bil,srok_mon,flg_bag,FLG_tuda_obr,flg_rab_day,kol_bil,plata,poteri,perebor,
	nedobor,nom,reg,d_plata,d_poteri,sf_pas,kol_pas,
	flg_ruch,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,
	FLG_BSP,FLG_SO,FLG_NU,FLG_TT,klass,kod_lgt,lgt_reg,
