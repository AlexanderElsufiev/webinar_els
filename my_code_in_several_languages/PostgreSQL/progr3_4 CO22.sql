...
with
yymm as (select max(yyyymm) as yymm from l3_mes.prig_times where oper='co22 read' and dann='prig' and itog is null),
...
--  select * from l3_mes.prig_times where oper='read' and dann='prig' and itog is null 

insert into l3_mes.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),'prig' as dann,'co22 beg' as oper,202212 as yyyymm,0 as rezult


select  * from l3_mes.prig_times where dann='prig' and itog is null and yyyymm=202212
...
(select row_number() over (order by idnum,nom_bil) as nom_str,idnum,request_num,
 ...
 from l3_mes.prig_itog where yymm=2304
),   
