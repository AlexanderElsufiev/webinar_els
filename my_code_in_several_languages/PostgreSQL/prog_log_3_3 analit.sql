...
--  DROP PROCEDURE svod.prig_agr_analit(text)

CREATE OR REPLACE PROCEDURE svod.prig_agr_analit(load_ text)
...
--- ввод в лог времени начала операции и её даты
execute '
--- ввод времени начала операции
delete from '||load_||'.prig_times where oper=''to_agr_analit'';

insert into  '||load_||'.prig_times(date_zap,part_zap,shema,rezult,dann,libr,oper,date,time,time2)
with times as (select * from '||load_||'.prig_times where dann=''prig'' and itog is null and substr(shema,5,4)=''prig'')
-- С заплаткой - данные по скоростным электричкам не обрабатывать. брать только чистый пригород
select date_zap,part_zap,shema,rezult,dann,libr,''to_agr_analit'' as oper,date,time,
...
execute 'select date_zap,part_zap from  '||load_||'.prig_times where oper=''to_agr_analit'' '
		into date_zap,part_zap;

RAISE INFO ' Начата агрегация постанционная За % дату % порция',  date_zap, part_zap;
...
(select 
 ...
 from '||load_||'.prig_itog where part_zap in (select part_zap from '||load_||'.prig_times where dann=''prig'' and oper=''to_agr_analit'')
),
...
from '||load_||'.prig_bil where nom_bil in (select distinct nom_bil from  prig)),
...
from '||load_||'.prig_dats where nom_dat in (select distinct nom_dat from  prig)) as a),	
...
RAISE INFO 'Закончена агрегация постанционная За % дату % порция, итог = % записей',  date_zap, part_zap, rezult;
...
insert into  '||load_||'.prig_times(date_zap,part_zap,shema,rezult,dann,libr,oper,date,time,time2)
with 
dat as (select date_zap,part_zap,dann,libr,shema from '||load_||'.prig_times where oper=''to_agr_analit'')

select date_zap,part_zap,shema,rezult,dann,libr,oper,date,time,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2
from
(select date_zap,part_zap,shema,dann,libr,'||rezult||' as rezult,''write_analit'' as oper,
 current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time
 from dat) as c;
		
update '||load_||'.prig_times set oper=''read_analit'' where oper=''to_agr_analit'';
...
ALTER PROCEDURE  svod.prig_agr_analit(text) OWNER TO asul; --компилляция!
...
--  call svod.prig_agr_analit('l3_prig'); --непосредственно запуск процедуры загрузки аналитической таблицы
	
--  call svod.prig_agr_analit('l3_mes'); --непосредственно запуск процедуры загрузки аналитической таблицы
