...
(select distinct date_zap from  spb_prig.prig_times where date_zap='2021-08-02'
 --where oper='dannie'
),
...
from rawdl2m.l2_prig_main   where REQUEST_DATE  in (select date_zap from dates)
			--  where REQUEST_DATE='2021-08-02' 
			 --and id in(310888340,310965615)
			),
...
from rawdl2m.l2_prig_cost where REQUEST_DATE in(select date_zap from dates) 
  	--where REQUEST_DATE='2021-08-02' and id=310877140 --УДАЛИТЬ!!!
),
...
hash_0 as
(select id,doc_num,mnom,srasst,hash1,sum(hash2) as hash2,
...
/*('x'||md5(hash1))::bit(64)::bigint as*/ hash1,('x'||md5(hash2))::bit(64)::bigint as hash2
from
(select id,doc_num,nom,mnom,srasst,reg,
 cast(sto as char(7))||'-'||cast(stn as char(7))||'-'||cast(srasst as char(7))||'-'||cast(sto_zone as char(7)) 
 ||'-'||cast(stn_zone as char(7)) ||'-'||cast(sti as char(7)) ||'-'||cast(sti_zone as char(7)) as hash1,
 cast(nom as char(7))||'-'||cast(st1 as char(7))||'-'||cast(st2 as char(7))||'-'||cast(rst as char(7))
 ||'-'||cast(marshr as char(7))||'-'||cast(reg as char(7))||'-'||cast(dorlin as char(7)) as hash2
from prig_cost3) as a) as b group by 1,2,3,4,5),


--old_hash as --Сделать выборку из справочника, когда будет создан
--(select mnom,hash1,hash2,1 as nom_mar from hash_0 where mnom=0),


old_hash as
(select nom_mar,srasst,hash1,sum(hash2) as hash2,max(nom) as mnom,
 sum(case when nom=1 then reg else 0 end) as sto_reg,sum(case when nom=mnom then reg else 0 end) as stn_reg from 
(select nom_mar,nom,srasst,reg,hash1,('x'||md5(hash2))::bit(64)::bigint as hash2,
...
(select nom_mar,nom,srasst,reg,
 ...
 ||'-'||cast(stn_zone as char(7)) ||'-'||cast(sti as char(7)) ||'-'||cast(sti_zone as char(7)) as hash1,
 cast(nom as char(7))||'-'||cast(st1 as char(7))||'-'||cast(st2 as char(7))||'-'||cast(rst as char(7))
 ||'-'||cast(marshr as char(7))||'-'||cast(reg as char(7))||'-'||cast((dor*10000+lin) as char(7)) as hash2
...
(select mnom,hash1,hash2,max(nom_mar) as nom_mar,min(nom_mar) as min_mar from
(select distinct mnom,hash1,hash2,-1 as nom_mar from hash_0
union
select mnom,hash1,hash2,nom_mar from old_hash) as a group by 1,2,3) as b),
...
(select id,doc_num,mnom,hash1,hash2,row_number() over (partition by mnom,hash1,hash2 order by id,doc_num) as nn
from hash_0) as a,all_hash as b where a.mnom=b.mnom and a.hash1=b.hash1 and a.hash2=b.hash2 and nn=1 and new_mar=1) as c join
...
(select c.id,c.doc_num,nom_mar,nom,mnom,srasst,reg,rst,d_plata,d_poteri,sto_reg,stn_reg
 from
(select id,doc_num,nom_mar,sto_reg,stn_reg from all_hash as a,hash_0 as b where a.mnom=b.mnom and a.hash1=b.hash1 and a.hash2=b.hash2) as c 
...
(select *,min(mi) over (partition by ndt) as min_mi,max(mi) over (partition by ndt) as max_mi,
 case when srok_mon>0 then round(k_pas/srok_mon) else 0 end as pas_mes
 from
(select ndt,dm,k_pas,srok_bil,srok_mon,min(i) as mi,count(*) as kol_d from dats_2 group by 1,2,3,4,5) as a)  as b)  as c) as d),
...
(select ndt,hash,mnom_dat+(count(*) over (order by ndt)) as nom_dat from
	(select ndt,hash from dats_hash where nom_dat=0 and mndt=ndt) as a,
...
(select a.ndt,nom_dat,i,kpas_day from dats_4 as a,dats_hash_new as b where a.ndt=b.ndt),
...
(select date_beg,k_pas,srok_bil,srok_mon,flg_rab_day,ndt,dt,kk,dm,i,
 count(*) over (partition by ndt,dm) as kol_day
 from
(select *,round(k_pas*nday/kday)-round(k_pas*(nday-1)/kday) as kk,
 	substr(cast(dt as char(10)),1,7) as dm
from
...
select * from dats2  order by ndt --order by -kol --order by kpas_day,dat

--85256600147757573497

--НЕ ВСЕ ВАРИАНТЫ!!!!! чТО-ТО ПОТЕРЯНО.

--select * from dats where ndt in( 219,431,626,643,202,315,656,127,294,115,529,621,411) order by ndt

--select * from prig_main where date_beg='2021-07-06' and k_pas=63 and srok_bil=90 and srok_mon=3 --flg_rab_day,i,dat,kpas_day

--ndt,date_beg,k_pas,srok_bil,srok_mon,flg_rab_day,i,dat,kpas_day
--101	"2021-07-06"	63	90	3	"2"	87	"2021-10-01"	3
