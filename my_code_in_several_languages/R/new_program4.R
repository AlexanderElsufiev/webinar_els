...
# РАЗВИТИЕ ПРОГРАММЫ  new_program3, только before станет из нескольких запаздываний
...
neuron$neir.get_neurs <- function (neir,nsets) {
  ...
  neurs=NULL;#koll=0
  neurs <- lapply(FUN = function(nset) {  #    for (nset in nsets){  # } # nset=6
    ...
    if (!is.na(nset)){
      ...
      { #выбрать нужный фильтр данных
        ...
        # поставить запаздывание каждой конкретной нейросети
        vh_=vh[(!is.na(vh$zap_vne)),]
        pol=neir$opis_pol
        pol=pol[(pol$n_pol %in% pols$n_pol),]
        pol$before=pol$before+pol$bef
        pol=pol[(!(pol$get %in% vh_$nm_)),]
        stat$before=NA;if (nrow(pol)>0){ stat$before=min(pol$before)}
      }
      ...
      #вычисление дат в настройке
      dats=unique(as.Date(dn[(!is.na(dn$yy)),'dat']))
      neur$stat$kol_dat=nrow(as.data.frame(dats))
      neur$stat$max_dat='2020-01-01'
      neur$stat$min_dat='2020-01-01'
      if (neur$stat$kol_dat>0){
        neur$stat$max_dat=as.character(max(dats))
        neur$stat$min_dat=as.character(min(dats))}
      
      
      # если мало данных, или ничего не прогнозируем - обнулить
      # далее не менее 10% дней и не менее 60% периода данных - тоже обнулить
      if ((neur$stat$dann_nastr<neur$stat$min_kol)|
          (neur$stat$dann_test<neur$stat$min_kol)|
          (neur$stat$dann_progn==0)|
          (neur$stat$kol_dat<(neuron$vhod$kol_dat/10))|
          (as.numeric(as.Date(neur$stat$max_dat)-as.Date(neur$stat$min_dat))<
           (as.numeric(as.Date(neuron$vhod$max_dat)-as.Date(neuron$vhod$min_dat))*0.6))
      ) {
...
neuron$neir.rez_err <- function (neir,plot=0,all=0) {
  # plot=0=не рисовать, =1=рисовать лучшие, =2=рисовать среднее от хороших
  # график качества итогов
  err=NULL;errs=NULL
  
  beff=neir$vhod$before
  
  if (all==0){# если надо взять лишь 1 случайный вариант
    beff$r=runif(nrow(beff));o=order(beff$r);beff=beff[o,];beff=beff[1,]
  }
  
  for (nom in beff$nom){#перебор всех вариантов      nom=3
    
    rez=neir$all_rezult
    bef=beff[(beff$nom==nom),];# nom=bef$nom;
    before=bef$before;row=bef$row
    
    rez$good=1*((rez$nomg1<=nom)&(rez$nomg2>=nom))
    rez$best=1*((rez$nomb1<=nom)&(rez$nomb2>=nom))
    for (nm in c('nomg1','nomg2','nomb1','nomb2','nom','before')){rez[,nm]=NULL}
    if (is.na(row)){row=max(rez$row)+1}
    ...
    if (is.null(row)) {row=max(dn[(as.Date(dn$dat)<=dat+before),'row'])+1}
    dn_=dn[(as.Date(dn$dat)>dat-2*before),];row_bef=NA  # для последних 2 периодов
    if (nrow(dn_)>0){row_bef=min(dn_$row)};rm(dn_)
    ...
    if (plot=='dat'){ # график средних значений прогнозов и отклонений по дате
      ...
      rz=merge(rz,dn,by='row')
      #rz=rz[(!is.na(rz$yy)),];  #убрал чтобы видеть прогноз на будущее
      ...
      rz$t=1;rr=rz;rr$y=rr$yy # среднесут значение реального значения
      rz$t=2;rz$y=rz$z1;rr=rbind(rr,rz) # среднесут значение самого прогноза, среднее всех лучших
      rz$t=3;rz$y=rz$z2;rr=rbind(rr,rz) # среднесут значение среднего от 5 хороших
      rz$t=4;rz$y=rz$e1;rr=rbind(rr,rz) # среднесут ошибка лучших
      rz$t=5;rz$y=rz$e2;rr=rbind(rr,rz) # среднесут ошибка средних от 5 хороших
      rz$t=6;rz$y=rz$err_sr;rr=rbind(rr,rz) # среднесут значение объвленной среднеквадратичной ошибки 
      rr$dat=as.Date(as.character(rr$dat))
      graf=rr;graf_main=paste('before=',before,sep='')
      #plot(y=rr$y,x=rr$dat,col=rr$t,main=paste('before=',before,sep=''))
      rm(rez1,rez2,rz,rr)
    }
    ...
    if (plot==1) {
      ...
      graf=rr;graf_main=paste('before=',before,sep='')
      # plot(rr$yy,rr$zn,col=rr$og,main=paste('before=',before,sep=''))
    ...
    {#чтобы понять точность в последние 2*before дней
      ...
      if (nrow(rz)>0){rz$prg=5;rez_=rbind(rez_,rz)};rm(rz)}
    ...
    err_prog=NA;err_prog2=NA;err_posled=NA
    if (nrow(err[(err$prg==1),])>0){err_prog=err[(err$prg==1),'err_sr']}
    if (nrow(err[(err$prg==2),])>0){err_prog2=err[(err$prg==2),'err_sr']}
    if (nrow(err[(err$prg==5),])>0){err_posled=err[(err$prg==5),'err_sr']}
    err$err_prog=err_prog;
    err$err_prog2=err_prog2;
    err$err_posled=err_posled
    ...
    {# добавить средние значения по всем просто хорошим, а не только лучшим
      ...
      if (nrow(rez)>0){
        ...
        if (plot==2) {
          graf=rez;graf_main=paste('before=',before,sep='')
          #plot(rez$yy,rez$zn,main=paste('before=',before,sep='')) 
        }# все лучшие итоги - график
      }}
    ...
    corr=paste('corr/d_er=(',round(err$c_yz*100000)/1000,'/',round(err$d_er*100000)/1000,')%',sep='')
    ...
    print(paste( ### 'errors: sum=',round(err$sum_err),'/',round(err$sum_err2),
      ...
      ' prg1/2=',round(err$err_prog*rr)/rr,'/',round(err$err_prog2*rr)/rr,'/ ',corr,sep=''))
    
    err$before=before;err$nom=nom
    errs=rbind(errs,err)
    
    
    if (plot==1) {
      plot(graf$yy,graf$zn,col=graf$og,main=paste(graf_main,' ',corr,sep=''))
    }# все лучшие итоги - график
    if (plot=='dat') {
      plot(y=graf$y,x=graf$dat,col=graf$t,main=paste(graf_main,' ',corr,sep=''))
    }
    if (plot==2) {
      plot(graf$yy,graf$zn,main=paste(graf_main,' ',corr,sep=''))
    }
    
  }#перебор всех вариантов конец
  
  return(errs)
  zn=1;rr=1;c_yz=1;d_er=1;d_y=1;d_z=1;dy=1;dz=1;e_yz=1;o=1;yy=1;err_posled=1;row_bef=1;bef=1;all=1;nom=1;graf=1;graf_main=1;corr=1;
  rm(k,rez,nm,nsets,plot,dd,dn,proc,zn,err,rez_,before,dat,err_prog,err_prog2,row,rr,ee)
  rm(c_yz,d_er,d_y,d_z,dy,dz,e_yz,o,yy,err_posled,row_bef,bef,all,nom,errs,beff,graf,graf_main,corr)
  #rm(neir)
}
...
neuron$neir.prognoz <- function (neir,all=0,okrugl=TRUE) {
  ...
  before=neir$vhod$before;rezz=NULL
  for (nom in before$nom){   ##  nom=0
    ...
    rez$best=1*((rez$nomb1<=nom)&(rez$nomb2>=nom))
    rez=rez[(rez$good>0),]
    for (nm in c('nomg1','nomg2','nomb1','nomb2','nom')){rez[,nm]=NULL}
    ...
    bef=before[(before$nom==nom),'before']
    if (nrow(rez)>0){rez$before=bef}
    
    
    rezz=rbind(rezz,rez)
  }# конец перебора вариантов before
  ...
  rez_=1;errs=1;errs_=1;ost_=1;ost2=1;i=1;proc=1;okrugl=1;pzd=1;dn=1;before=1;rezz=1;bef=1;nom=1;
  rm(dd,dn,mas,mass,ogr,rez,rez_,vh,errs,nm,nm_,errs_,all,ost,ost_,ost2,i,proc,okrugl,pzd)
  rm(neir,before,bef,rezz,nom)
}
...
neuron$neir.prognozi_save <- function (neir) {
  ...
  ver=neir$vhod$version;nms=c('name','max_dat','before','yy')
  if (('version' %in% names(prognoz)) |(!is.null(ver))){
    nms=c(nms,'version');
    if (is.null(ver)) {ver=NA}
    progn$version=ver
    if (!('version' %in% names(prognoz))) {prognoz$version=NA}
  }
  ...
  if (!is.null(prognoz)){
    ...
    prognoz=merge(prognoz,pp,by=nms,all=TRUE)
    prognoz=prognoz[(is.na(prognoz$iz)),]
    prognoz$iz=NULL}
  
  prognoz=myPackage$sliv(prognoz,progn)
  myPackage$trs.Data_save(prognoz,'prognoz') #запись на диск прогнозов
  rm(max_dat,name,yy,pp,progn,prognoz,nm,nms,ver,neir)
}
...
neuron$neir.zapis_neurs<- function (neir,neurs) {
  ...
  if ('nom_nastr' %in% names(neir$errors)){nom_nastr=max(neir$errors$nom_nastr)+1}}
  ...
  for (neur in neurs){   #    for (neur in neurs){ print(names(neur));if ('dn' %in% names(neur)) {nnn=neur}};neur=nnn;rm(nnn)   names(neur)
    nom_neur=nom_neur+1
    ...
    if (!is.null(neur)){
      ...
      if (stat$act %in% c(2,4)) {
        ...
        if (!is.null(rez)){ ## пока не проверено!!!
          rez=rez[,c('row','yy','zn','err','nset',neuron$vhod$errs)]
          rez[,c('nomg1','nomg2','nomb1','nomb2','nom')]=NA
          rez$before=stat$before
  ...
  #   rm(itog,mass,param,pols,stat,str,rez)
  ...
  if (zap_rez>0) { ## пока не проверено - нечем!
    neir=neuron$neir.kol_good_best(neir) }
  
  all_stat=neir$all_stat;all_itogi=neir$all_itogi;
  if (is.null(all_stat$kol_best)){all_stat$kol_best=0}
  all_stat[(is.na(all_stat$kol_best)),'kol_best']=0
  ...
  o=(all_stat$act==4)&(all_stat$kol_best==0) #&(all_stat$act==2)
  if (nrow(all_stat[o,])>0){ #удаление лишних результатов
    all_stat[o,'act']=3;   # признак - больше не настраивать   
    nset=all_stat[o,'nset']
    rez=neir$all_rezult
    rez=rez[(!(rez$nset %in%nset)),]
    neir$all_rezult=rez
    rm(rez,nset)
  }
  ...
  if (FALSE)  {# если ни разу не лучший, и очень мало хороший - удалить
    all_stat$act_=1
  ...
  rm(all_stat,all_itogi,it,it_,neir,neurs,neur,o,stat,err)
...
neuron$neir.init_vhodi <- function (kol_pol=0) {
  ...
  before=before[(!is.na(before$before)),]
  
  if (kol_pol==0){
    {# придумать исходно все возм входы
      
      pp=xx;pp$get=pp$nm_;# pp$before=bef;
      {# pp$before=bef; - когда много запаздываний
        pp$ed=1;before$ed=1;before=before[,c('before','ed')]
        pp=merge(pp,before,by='ed');pp$ed=NULL}
      ...
      if (min(before$before)<364) {  #добавка входов год назад
      ...
      {#округления до 7-дневной недели
        ppv=ppv[(ppv$get!='upr'),];
        ppv$before=round((ppv$before+3)/7)*7
        pp=rbind(pp,ppv)
        ppv$before=ppv$before-ppv$bef;pp=rbind(pp,ppv)
        pp=unique(pp);rm(ppv,pp_)}
    }
    ...
    for (nn in (1:kol_pol)){ #  nn=1
      ...
      if (tip=='xs'){pol$zapazd=round(runif(1)*min(before$before))+2}
      ...
      pol$before=max(0,before[(max(1,runif(1)*nrow(before))),'before'] +round(runif(1)*30)-bef) #запаздывание
      ...
      if ((min(before$before)<364)&(runif(1)<0.3)){# подстановка даты с год назад
        pol$before=364+round(runif(1)*5)*7
      }
  ...
  mas_=1;pps=1;nnm=1;tm=1;tms=1;is=1;k=1;nms=1;before=1
  
  return(poll)
  rm(dann,dn,mas,pol,poll,vhodi,xx,get,kol_pol,nm,nn,o,tip,zn,has,xx_,bef,mass,ms)
  rm(mas_,pps,nnm,tm,tms,pp,pp_,nom,is,k,nms,before)
}
...
neuron$neir.plus_nsets <- function (neir,kol_plus) {
  ...
  if (nrow(stat)>0){
    ...
    if (is.null(stat$kol_good)){stat$kol_good=0}
...
neuron$neir.plus_vhodi <- function (neir) {
  ...
  { # часть вторая - лучшие входы добавить в изменённом виде - убрать по 1 фильтру, или (before zapazd) кратны 7
    ...
    for (npol in pol_$n_pol){ #  для каждого входа - по очереди вычеркнуть 1 из ограничений  npol=110
      ...
      bef=neir$vhod$before;bef=bef[(!is.na(bef$before)),];bef=min(bef$before)
      pp$before=max(round(pp$before/7)*7,bef)
      pol_=rbind(pol_,pp)
      if (!is.na(pp$zapazd)){
  ...
  {#РАССМОТРЕТЬ 5 ЛУЧШИХ НЕЙРОСЕТЕВЫХ ВХОДОВ, И СДЕЛАТЬ СЛАБЫЕ МУТАЦИИ, ВСЕ ВОЗМОЖНЫЕ
    ...
    {#НАЙТИ ВСЕ ПОХОЖИЕ ЗАМЕНИТЕЛИ! - или поставить новых (слабые мутации)
      ...
      {# отличаются запаздыванием
        bef=neir$vhod$before;bef=bef[(!is.na(bef$before)),]
        beff=c((1:7),14,21,28,bef$before,364)
    ...
    {#убрать теперь лишние строки
      ...
      o=((is.na(pol_$zap_vne))&(pol_$before<min(bef$before)))
      pol_[o,'before']=min(bef$before)
...
neuron$neir.kol_good_best <- function(neir) {
  ...
  {# постановка номеров запаздываний - если не сделано исходно
    rez$nom=0
    for (nom in c(max(before$nom):min(before$nom))) {  #  nom=5
      bef=before[(before$nom==nom),'before']
      if (!is.na(bef)){
        o=((rez$before>=bef)&(!is.na(rez$before)));
        if (nrow(rez[o,])>0){rez[o,'nom']=nom}
      }}}
  
  {# для ускорения - разделение данных
    if (is.null(rez$nomb2)){rez[,c('nomb1','nomb2','nomg1','nomg2')]=NA }
    rows=unique(rez[(is.na(rez$nomb2)),'row'])
    o=(rez$row %in% rows);  ##  o=!o
    rez1=rez[(!o),];rez=rez[o,] # rez1=уже всё рассчитано  rez= на рассчёт best
    rezz=rez # список на все возможные запаздывания
    rezz[,c('nomg1','nomg2','nomb1','nomb2')]=-1;rez=rezz[0,]
  }
  
  # перебор последовательно всех возможных запаздываний прогноза
  for (nom in c(min(before$nom):max(before$nom))) {  #  }   nom=0     nom=nom+1     
    rr=rezz[(rezz$nom==nom),];kk=nrow(rr);rez=rbind(rez,rr);rm(rr)
    
    
    if (nrow(rez)>0) {
      
      if (kk==0){#если ничего не добавилось!   ############### ошибка - 0 записей!
        rez[(rez$nomg2==nom-1),'nomg2']=nom
        rez[(rez$nomb2==nom-1),'nomb2']=nom
      } else { #Если есть добавка
        ...
        for (nm in errs$name){ # перебор всех статистик ошибки, рассчёт good   
        ...
        {# поиск лучшего и 5 хороших
          o=order(rez$row,-rez$good,rez$nset);rez=rez[o,] 
          rez$n=(1:nrow(rez));rez$n=rez$n-rez$nn
          rez$best=0;rez$good=0;
          rez[(rez$n==0),'best']=1
          rez[(rez$n<=4),'good']=1
          rez$n=NULL;rez$nn=NULL;
        }
        { #запись best в память
          rez[(rez$best==1),'nomb2']=nom
          rez[(rez$best==1)&(rez$nomb1==-1),'nomb1']=nom
          rez[(rez$good==1),'nomg2']=nom
          rez[(rez$good==1)&(rez$nomg1==-1),'nomg1']=nom
          
          rez$best=NULL;rez$good=NULL
        }
        {#Убрать лишние записи
          rr=rez[(rez$nomg2<nom),];rez1=rbind(rez1,rr)
          rez=rez[(rez$nomg2==nom),];rm(rr)
        }
        
      }#конец проверки - что-то добавилось
    }
  }#конец цикла по запаздываниям    
  
  
  rez=rbind(rez,rez1)
  rm(rez1,rezz)
  {# итоговыая статистика лучших и хороших
    rez$best=0;rez$good=0;
    o=(rez$nomb2>=0)
    rez[o,'best']=(rez[o,'nomb2']-rez[o,'nomb1']+1)
    o=(rez$nomg2>=0)
    rez[o,'good']=(rez[o,'nomg2']-rez[o,'nomg1']+1)
    
    rezz=aggregate(x=subset(rez,select=c('best','good')),by=subset(rez,select=c('nset')), FUN="sum" )
    stat=merge(stat,rezz,by='nset',all=TRUE)
    o=(stat$act!=1)
    stat[o,'kol_best']=stat[o,'best'];stat[o,'kol_good']=stat[o,'good'];
    stat[(is.na(stat$kol_best)),c('kol_best')]=0 # были и нету - всё по нулям
    stat[(is.na(stat$kol_good)),c('kol_good')]=0 # были и нету - всё по нулям
    stat$best=NULL;stat$good=NULL
  }
  
  
  # уменьшить объём памяти
  rez=rez[(rez$good>0),]
  rez$good=NULL;rez$best=NULL
  rez$nset=as.integer(rez$nset);rez$row=as.integer(rez$row);
  rez=as.data.frame(as.data.table(rez))
  rez$nom=NULL
  neir$all_rezult=rez;neir$all_stat=stat
  
  return(neir)
  rez=1;rezz=1;rr=1;stat=1;errs=1;nm=1;o=1;rows=1;proc=1;neir=1;rez1=1;before=1;bf=1;bef=1;kk=1;nom=1;
  rm(rez,rezz,rr,stat,errs,nm,o,rows,proc,neir,rez1,before,bf,bef,kk,nom)
}
...
neuron$neir.load_from_hist <- function(nname='') {
      ...
      neir=neuron$neir.init_neir() } else{
        ...
        if (nrow(mm)!=nrow(mas1)){# реально надо менять
          ...
          for(nm in mas$name){
            ...
            pol=pol[(!is.na(pol$nom_n)),] #убрал записи, которым нет соответствия
  ...
  {# Если уменьшилось число параметров массива - убрать и соотв нейросети
    pol=neir$opis_pol
    poll=neir$opis_poll
    pols=neir$opis_pols
    bpols=unique(pols[(!(pols$n_pol %in% pol$n_pol)),'n_poll'])
    pols=pols[(!(pols$n_poll %in% bpols)),]
    poll=poll[(!(poll$n_poll %in% bpols)),]
    neir$opis_poll=poll
    neir$opis_pols=pols
    
    fil=neir$all_filtr
    fil=fil[(!(fil$n_poll %in% bpols)),]
    neir$all_filtr=fil
    
    pp=neir$all_pols
    pp=pp[(pp$nset %in% fil$nset),]
    neir$all_pols=pp
    ...
    rm(pol,poll,pols,fil,pp,bpols,stat)
  }
  ...
  if (FALSE){ # НАВЕЧНО УБИЛ - возможно понадобится. при расширении/сужении числа настраиваемых запаздываний
