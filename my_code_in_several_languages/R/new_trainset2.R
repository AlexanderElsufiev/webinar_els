...
myPackage$trs.tData.extractor <- function(pData, mData, wData) {
  ...
  pDate <- as.data.table(pData)
  pDate$Before <- pmin(pmax(pDate$Before,1),45);
  pDate$Pkm <- pDate$Kol_pas * pDate$Rasst;
  cat(paste("Before aggregate 1: ", as.character(Sys.time()), "\n"))
  marshr <- pDate[, lapply(.SD, function(x){sum(abs(x))}), 
  ...
  rst <- myPackage$marshr_to_rasst(marshr);
  
  max_rst <- max(rst$rst)
  
  rst_o <- data.frame(Sto = rst$kst, n_o = rst$nom)
  rst_n <- data.frame(Stn = rst$kst, n_n = rst$nom)
  rst <- as.data.table(merge(rst_o, rst_n, by = c()))
  
  mesta <- merge(pDate, rst, by = c('Sto', 'Stn'))
  rm(rst,rst_o,rst_n)
  
  cat(paste("Before aggregate 2: ", as.character(Sys.time()), "\n"))
  mesta <- mesta[, lapply(.SD, sum, na.rm = T), .SDcols = "Kol_pas",
                 by = c('Train', 'Date', 'Type', 'Klass', 'n_o', 'n_n')]
  cat(paste("After aggregate 2: ", as.character(Sys.time()), "\n"))
  pDate[(pData$Rasst < max_rst), 'Cena'] = 0
  cat(paste("Before aggregate 3: ", as.character(Sys.time()), "\n"))
  cena <- pDate[, lapply(.SD, max, na.rm = T), .SDcols = c("Cena"),
                by = c('Date', 'Train', 'Type', 'Klass', 'Arenda', 'Before')]
  cat(paste("After aggregate 3: ", as.character(Sys.time()), "\n"))
  cat(paste("Before aggregate 4: ", as.character(Sys.time()), "\n"))
  pass <- pDate[, lapply(.SD, sum, na.rm = TRUE), by = c('Date','Train','Type','Klass','Arenda','Before'),
                .SDcols = c("Kol_pas", "Stoim", "Pkm")]
  cat(paste("After aggregate 4: ", as.character(Sys.time()), "\n"))
  pass <- merge(pass, cena , by = c('Date','Train','Type','Klass','Arenda','Before'))
  pass_ <- pass[(pass$Klass != '-'), ];#добавка сумм классов вагона
  if (nrow(pass_) > 0) {
    pass_$klass <- '-'
    cat(paste("Before aggregate 5: ", as.character(Sys.time()), "\n"))
    cena <- pass_[, lapply(.SD, max, na.rm = TRUE), .SDcols = c("Cena"),
                  by = c('Date','Train','Type','Klass','Arenda','Before')]
    cat(paste("After aggregate 5: ", as.character(Sys.time()), "\n"))
    cat(paste("Before aggregate 6: ", as.character(Sys.time()), "\n"))
    pass_ <- pass_[, lapply(.SD, sum, na.rm = TRUE), by = c('Date','Train','Type','Klass','Arenda','Before'),
                   .SDcols = c("Kol_pas", "Stoim", "Pkm")]
    cat(paste("After aggregate 6: ", as.character(Sys.time()), "\n"))
  ...
  if (nrow(pass_) > 0) {
    pass_$Type <- '-'
    cat(paste("Before aggregate 7: ", as.character(Sys.time()), "\n"))
    cena <- pass_[, lapply(.SD, max, na.rm = TRUE), .SDcols = c("Cena"),
                  by = c('Date','Train','Type','Klass','Arenda','Before')]
    cat(paste("After aggregate 7: ", as.character(Sys.time()), "\n"))
    cat(paste("Before aggregate 8: ", as.character(Sys.time()), "\n"))
    pass_ <- pass_[, lapply(.SD, sum, na.rm = TRUE), by = c('Date','Train','Type','Klass','Arenda','Before'),
                   .SDcols = c("Kol_pas", "Stoim", "Pkm")]
    cat(paste("After aggregate 8: ", as.character(Sys.time()), "\n"))
    pass_ <- merge(pass_,cena,by = c('Date','Train','Type','Klass','Arenda','Before'))
  }
  pass <- rbind(pass,pass_)
  pDate <- pass;
  ...
  cat(paste("Before first split: ", as.character(Sys.time()), "\n"))
  f <- function(.SD) {
    kp <- numeric(45);
    rent <- numeric(45);
    pkm <- numeric(45);
    ...
    len <- nrow(.SD);
    total <- 0;
    r_total <- 0;#rasst=max(part$Rasst)
    before <- .SD$Before
    ar <- .SD$Arenda
    bkp <- .SD$Kol_pas
    bpkm <- .SD$Pkm
  ...
  q <- pDate[, f(.SD), by = c('Date', 'Train', 'Type', 'Klass')]
  ...
  rm(parsed, q, columns)
  cat(paste("After first split: ", as.character(Sys.time()), "\n"))
