...
data cena;seed=round(&seed);do depo=1 to &kol_depo;
        ...
        call ranuni(seed,c);c_br=10*(c+5);/*цена жизни лок.бригады, за сутки (её работа и холостой ход - входят в локомотив)*/
...
/*ставлю максимальный холостой пробег не длиннее максимального перегона в данном депо*/
...
if tb='0' then do;seed=0;variant_=0;variant=&variant;time=0;maxx=&maxx;put 'ВХОД';
...
flag2:;/*поиск очередного маршрута*/rasp2=0; nnn='0';/*put 'вх ' nn= rasp1= depo=;*/
...
/*параметры проезда по перегону*/mars_end=0;
