...
##############################################################################
###  НАДО СМОДЕЛИРОВАТЬ ПРОДАЖИ САПСАНОВ (10 ЛЕТ) С ИЗМЕНЯЕМЫМИ ЦЕНАМИ


# 1. число разных групп пассажиров, со своими приоритетами
pas_grup=10
# 2. количество конкурирующих поездов в сутки, с постоянными интервалами между ними, 
#       и одинаковым числом вагонов
kol_train=5
before=45
year=7*50

wd=round(30*(sin(4:10)+2))
t=(1:50)*2*3.141592/50
week=round(30*(sin(t)+cos(3*t)/2 +cos(7*t)/4 +3));



grup=data.frame(grup=array(0,pas_grup))
for (gr in(1:pas_grup)){
  grup[gr,'k_cen']=1/(gr+2) #больше номер - более состоятельные
  grup[gr,'grup']=gr
  for (w in (1:7)){
    grup[gr,paste('wd',w,sep='')]=wd[w]+round(5*sin(gr*w))}
  for (w in (1:50)){
    grup[gr,paste('week',w,sep='')]=week[w]+round(11*sin(gr*w+15))}
  for (w in (1:kol_train)){
    grup[gr,paste('tr',w,sep='')]=round(15+10*sin(gr*w))}
  for (w in (1:45)){
    grup[gr,paste('bef',w,sep='')]=(w+gr)**(-gr/pas_grup)   }
  }
...
# ИЗНАЧАЛЬНАЯ ЦЕНА БЫЛА КОНСТАНТА, =1000
dann=NULL
dn=data.frame(day=0)
...
for (day in (1:(3*year))){
 ...
 dn$day=day; 
  for (bef in (1:before)){dn$bef=bef;dat=day+before;dn$dat=dat;
    wd=dat-round((dat-4)/7)*7;week=(dat-wd)/7+1
    while(week>50){week=week-50};if (week<0){week=week+50}
 
    for (tr in(1:kol_train)){dn$tr=tr;kp=0
    for (gr in (1:pas_grup)){
    kp=kp+(grup[gr,paste('wd',wd,sep='')]+grup[gr,paste('week',week,sep='')])*
      grup[gr,paste('tr',tr,sep='')]*grup[gr,paste('bef',bef,sep='')]*(1000-cena*grup[gr,'k_cen'])

    } 
    dn$kp=kp;
    dann=myPackage$sliv(dann,dn)
    }}
  }
