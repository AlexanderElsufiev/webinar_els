...
--  DROP PROCEDURE svod.prig_co22(text)

CREATE OR REPLACE PROCEDURE svod.prig_co22(load_ text)
...
declare
	rezult integer;rez integer;
	yyyymm integer;yymm integer;
	date_zap	date;
	load_ text;
...
WHILE  (rezult>0) LOOP  --3  цикл пока есть что читать
rezult=0;


execute 'with
yymm as (select max(yyyymm) as yymm from '||load_||'.prig_times where oper=''co22 read'' and dann=''prig'' and itog is null),
rd as (select min(yyyymm) as yymm1,max(yyyymm) as yymm2
	   from '||load_||'.prig_times where oper=''read'' and dann=''prig'' and itog is null and yyyymm> (select coalesce(yymm,0) from yymm))
select yymm1 from rd where yymm1<yymm2'
		into yyyymm;

if yyyymm is not null then begin

yymm=yyyymm-200000;
RAISE INFO ' Начата расчёт ЦО-22 За % месяц',  yyyymm;

execute '
--удаление всего лишнего
delete from '||load_||'.prig_times
where ''prig''=dann and ''co22''=substr(oper,1,4) and '||yyyymm||'= yyyymm;

delete from '||load_||'.prig_co22_t0;

--- ввод времени начала операции
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 beg'' as oper,'||yyyymm||' as yyyymm,0 as rezult;
';
...
insert into '||load_||'.prig_co22_t0 
...
(select row_number() over (order by idnum,nom_bil) as nom_str,idnum,request_num,
 ...
 from '||load_||'.prig_itog where yymm='||yymm||'
),   
...
(select date_pr,date_beg,a.nom_dat,plus_dat,kpas_day,date_beg+plus_dat as date_otpr
 from dat1 as a, '||load_||'.prig_dats as b where a.nom_dat=b.nom_dat and kpas_day!=0), -- as a),
...
(select max(nom) over(partition by nom_mar) as max_nom,*
 from '||load_||'.prig_mars where nom_mar in (select distinct nom_mar from prig)),
...
--case when FLG_OFFICIAL_BENEFIT in (''Д'',''Ф'') then ''1'' else FLG_OFFICIAL_BENEFIT end as flg_zd_fpk_f
 ...
 from '||load_||'.prig_bil as a left join nsi.site as b on a.web_id=b.idsite and current_date between datan and datak 
),
...
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей',  yyyymm, rezult;
rez=rezult;

RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 1',  yyyymm, rezult;

execute '
--- ввод итога расчёта общей части
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t0_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;
';
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 2',  yyyymm, rezult;
...
execute '
insert into '||load_||'.prig_co22_t1 
...
p21,p22,p23,p24,p25,p26,p27,p28,p29,p30,p31,p32,p33,p34,p35,p36,p37,p38,p39,p40,								 
p41,p42,p43,p44,p45,p46,p47,p48,p49,p50,p51,p52,p53,p54,p55,p56,p57,p58,p59,p60,p61,p62,p63
from '||load_||'.prig_co22_t0 where p1=''tab1'';
';
GET DIAGNOSTICS rezult := ROW_COUNT;
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 3',  yyyymm, rezult;

commit;
execute '
--- ввод итога расчёта таблицы 1
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t1_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;';
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 4',  yyyymm, rezult;

execute '
-----------------
insert into '||load_||'.prig_co22_t2(yymm,p1,p2,p3,p4,p5,p6,p7)
select yymm,p1,p5 as p2,p2 as p3,d1 as p4,d2 as p5,d3 as p6,d4 as p7
from '||load_||'.prig_co22_t0 where p1=''tab2'';
';
GET DIAGNOSTICS rezult := ROW_COUNT;commit;
execute '
--- ввод итога расчёта таблицы 2
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t2_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;';

RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 5',  yyyymm, rezult;
execute '
-----------------
insert into '||load_||'.prig_co22_t3(yymm,p1,p2,p3,p4,p5,p6,p7)
select yymm,p1,p5 as p2,p2 as p3,d1 as p4,d5 as p5,d6 as p6,d4 as p7
from '||load_||'.prig_co22_t0 where p1=''tab3'';
';
GET DIAGNOSTICS rezult := ROW_COUNT;
commit;
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 6',  yyyymm, rezult;
execute '
--- ввод итога расчёта таблицы 3
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t3_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;';

execute '
-----------------
insert into '||load_||'.prig_co22_t4
(yymm,p1,p2,p3,p4,p5,p6,p7,p8,p9)
select yymm,p1,p5 as p2,p2 as p3,d1 as p4,d2 as p5,d6 as p6,p36 as p7,p44 as p8,d4 as p9
from '||load_||'.prig_co22_t0 where p1=''tab4'';
';
GET DIAGNOSTICS rezult := ROW_COUNT;
commit;
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 7',  yyyymm, rezult;
execute '
--- ввод итога расчёта таблицы 4
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t4_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;';

execute '
-----------------
insert into '||load_||'.prig_co22_t5(yymm,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10)
select yymm,''tab5'' as tab_1,a.p5 as kod_vc_2,kol1 as kol1_3,kol2 as kol2_4,kol3 as kol3_5,kol_pas as kol_pas_6,0 as sum_7,0 as sum_8,plata as plata_9,poteri as poteri_10
 ...
 (select yymm,p5,count(*) as kol1,sum(p33) as kol_pas,sum(p36) as plata,sum(p44) as poteri
	   from '||load_||'.prig_co22_t0 where p1=''tab1'' group by 1,2) as a
 join (select p5,count(*) as kol2 from '||load_||'.prig_co22_t0 where p1=''tab2'' group by 1) as b on a.p5=b.p5
 join (select p5,count(*) as kol3 from '||load_||'.prig_co22_t0 where p1=''tab3'' group by 1) as c on a.p5=c.p5
 join (select p5,count(*) as kol4 from '||load_||'.prig_co22_t0 where p1=''tab4'' group by 1) as d on a.p5=d.p5
;
';
GET DIAGNOSTICS rezult := ROW_COUNT;
commit;
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 8',  yyyymm, rezult;

execute '
--- ввод итога расчёта таблицы 5
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t5_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;';
execute '
-----------------
insert into '||load_||'.prig_co22_t6(yymm,p1,p2,p3,p4,p5,p6,p7)
select yymm,p1,p5 as p2,p2 as p3,d1 as p4,d2 as p5,d7 as p6,d4 as p7
from '||load_||'.prig_co22_t0 where p1=''tab6'';
';
GET DIAGNOSTICS rezult := ROW_COUNT;
commit;
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 9',  yyyymm, rezult;
execute '
--- ввод итога расчёта таблицы 6
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 t6_end'' as oper,'||yyyymm||' as yyyymm,'||rezult||' as rezult;

--ПРИЗНАК ОКОНЧАНИЯ РАСЧЁТА
insert into '||load_||'.prig_times(date,time,dann,oper,yyyymm,rezult)
select current_date,cast(current_time as char(12)),''prig'' as dann,''co22 read'' as oper,'||yyyymm||' as yyyymm,'||rez||' as rezult;

';
RAISE INFO 'Загрузка ЦО-22 за % месяц, первая часть закончена, итог = % записей 10',  yyyymm, rezult;
	
	
execute 'select count(*) from '||load_||'.prig_times where substr(oper,1,4)=''co22'' '
		into rezult;	
RAISE INFO 'ВСЕГО ЗАПИСАНО % записей',  rezult;	
...
ALTER PROCEDURE  svod.prig_co22(text) OWNER TO asul; --компилляция!
...
--   call svod.prig_co22('l3_prig'); --непосредственно запуск процедуры загрузки аналитической таблицы
	
--  call svod.prig_co22('l3_mes'); --непосредственно запуск процедуры загрузки аналитической таблицы
						

select * from l3_prig.prig_times where substr(oper,1,4)='co22'

select * from l3_mes.prig_times where substr(oper,1,4)='co22'
