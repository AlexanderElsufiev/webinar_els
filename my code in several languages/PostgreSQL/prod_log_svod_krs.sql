...
-- DROP PROCEDURE svod.svod_krs(text)


CREATE OR REPLACE PROCEDURE svod.svod_krs(load_ text)
...
--RAISE INFO ' Обработка данных для свода по КРС, рабочая схема записи=(%)',  load_;	  
...
RAISE INFO ' СВОД ПО КРС. Время Начала=% рабочая схема записи=%   схема чтения %  номер порции данных=% объём порции записей=%',  calc_time, load_, libr, part_zap, rezult;		
...
with 
dats as (select min_id_svod,date_zap,min_id,max_id from '||load_||'.prig_times where oper=''dannie'' and dann=''svod_krs''),
...
-- ЗАГРУЗКА ОСНОВНОЙ ТАБЛИЦЫ СВОДА  КРС
execute ' 	
...
GET DIAGNOSTICS rezult := ROW_COUNT;
RAISE INFO 'ШАГ 2. Записано % записей в основную таблицу свода по КРС. За % дату % порция', rezult,date_zap, part_zap;	
...
GET DIAGNOSTICS rezult := ROW_COUNT;
RAISE INFO 'ШАГ 3. Записано % записей в таблицу линков свода по КРС. За % дату % порция', rezult,date_zap, part_zap;	
...
end  if;--if dann='svod_krs' then begin --начало работы если надо							
...
ALTER PROCEDURE svod.svod_krs(text) OWNER TO asul; --компилляция!


--    call svod.svod_krs('l3_mes');    --непосредственно запуск процедуры
