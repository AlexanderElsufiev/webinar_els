...
-- DROP PROCEDURE svod.prig_obog_4(text)

CREATE OR REPLACE PROCEDURE svod.prig_obog_4(load_ text)

    LANGUAGE 'plpgsql'
    
AS $BODY$
--ТОЛЬКО ПРОЦЕДУРА ОБОГАЩЕНИЯ, ЧЕТВЁРТАЯ ПОСЛЕДНЯЯ ЧАСТЬ ОБРАБОТКИ - ЗАГРУЗКА В ИТОГИ
--ПЕРЕГОН ПОЛУЧЕННЫХ ДАННЫХ В СПРАВОЧНИКИ БИЛЕТОВ, ДАТ И МАРШРУТОВ, И ИТОГ ОБОГАЩЕНИЯ
...
execute 'select date_zap,part_zap,cast(current_time as char(50)) from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'' '
		into date_zap,part_zap,calc_time;



--- ввод времени начала операции
execute '
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr)
with a as
...
select time,date,dann,''work 2_4_beg'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,date_zap,part_zap,shema,libr
from a;
';
--commit;
...
execute '
insert into '||load_||'.prig_dats 
(nom_dat,plus_dat,kpas_day,idnum,date_zap,part_zap) 
select nom_dat,kol_bil as plus_dat,k_pas as kpas_day,idnum,date_zap,part_zap from '||load_||'.prig_work where rez=1;
';

GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ ДАТ % записей. За % дату. Время Начала=%', integer_var, date_zap,calc_time;	
--commit;

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_dats'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,
date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
	from  '||load_||'.prig_times  where dann=''prig'' and oper=''dannie'') as a
';
--commit;
...
from '||load_||'.prig_work where rez=2 ; 
...
GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ БИЛЕТОВ % записей. За % дату', integer_var, date_zap;	

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_bil'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,
	date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
...
execute ' 
insert into '||load_||'.prig_peregoni(dor,lin,st1,st2,rasst,peregon,date_zap,part_zap)
with 
per1 as
(select dor,lin,st1,st2,min(date_zap) as date_zap,min(rst) as rst,min(part_zap) as part_zap from 
(select distinct dor,lin,st1,st2,rst,date_zap,part_zap from '||load_||'.prig_work where rez=8) as a
...
(select a.dor,a.lin,a.st1,a.st2,a.rst,peregon,a.date_zap,a.part_zap
 from per1 as a left join '||load_||'.prig_peregoni as b on a.dor=b.dor and a.lin=b.lin and a.st1=b.st1 and a.st2=b.st2) as c
...
(select distinct dor,lin,st1,st2,rst,date_zap,part_zap,
 ...
 from per3 as a,
 (select max(peregon) as mper from '||load_||'.prig_peregoni) as b),
...
GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ ПЕРЕГОНОВ % записей. За % дату', integer_var, date_zap;	

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_pereg'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,
	date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
...
from '||load_||'.prig_work where rez=8) as a
join '||load_||'.prig_peregoni as b on a.dor=b.dor and a.lin=b.lin and a.st1=b.st1 and a.st2=b.st2
;
...
GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ МАРШРУТОВ % записей. За % дату', integer_var,date_zap;	
--commit;

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_mars'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,
	date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
...
execute '
insert into '||load_||'.prig_bad
(marshr,st1,st2,rst,reg,dor,lin,date_zap,part_zap,idnum)
select
marshr,st1,st2,rst,reg,dor,lin,date_zap,part_zap,idnum
from '||load_||'.prig_work where rez=99;
';
--commit;

GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ ОШИБОК МАРШРУТОВ % записей. За % дату', integer_var, date_zap;	

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_error'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,
date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
...
from '||load_||'.prig_work where rez=9),
...
GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ ОБОГАЩЕНИЯ *** % записей. За % дату', integer_var, date_zap;	

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_itog'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,
	date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
...
--Обогащение ДАННЫХ ПО ЛЬГОТНИКАМ ДЛЯ РЕЕСТРА ПО НОВОМУ
execute '


/**/
insert into '||load_||'.prig_lgot_reestr
...
bil0 as (select * from '||load_||'.prig_bil where kod_lgt>0 
...
ish as
(select idnum,yymm,nom_mar,nom_bil,nom_dat,date_zap,part_zap,date_beg,date_end,date_pr,TERM_DOR,term_pos,term_trm,time_zap,server_reqnum,
case when length(drac)<=5 then drac else substr(drac,length(drac)-4,5) end as drac,
...
from '||load_||'.prig_work where rez=9 and nom_bil in (select nom_bil from bil0)),

lgot_fio as
(select idnum,request_num,fio,fio_2,snils,ticket,kodbl,benefit_doc,benefit_podr,bilgroup
from '||load_||'.prig_work where rez=0 /*and snils is not null*/),
...
/*case when flg_fee_onboard=''1'' then ''kvi'' --квитанция за оформление в поезде
 when flg_bag=''1'' then ''bag'' --перевозоыный документ для багажа, впрос с 7=грузобагаж
 when prod in(''i'',''m'') then ''raz'' --интернет продажа 800-х АМГХИЙ разовые 
 when abonement_type=''0''  then ''raz'' --пригородные разовые
 else ''abn'' --все виды абонементов
 end as raz_per_16, */
...
(select max(nom) over(partition by nom_mar) as max_nom,*
 from '||load_||'.prig_mars where nom_mar in (select distinct nom_mar from lgot)),
...
/*case when  abonement_type>''0'' then 0 else*/ kol_bil /*end*/ as raz_per_16,
...
--GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСЬ В БАЗУ РЕЕСТРА ЛЬГОТНИКОВ, РАСЧЁТ ДАННЫХ  % записей. За % дату', integer_var, date_zap;	

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 

insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_lgot_ish'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
	from  '||load_||'.prig_times  where dann=''prig'' and oper=''dannie'') as a;
...
insert into '||load_||'.prig_work (yymm,date_zap,part_zap,rez)
select yymm,date_zap,part_zap,11 as rez from
(select yymm,date_zap,part_zap,count(*) from '||load_||'.prig_work  where rez=9 group by 1,2,3) as a;

--УДАЛЕНИЕ ИЗ КОНТРОЛЬНОЙ ТАБЛИЦЫ ИЗМЕНИВШИХСЯ ЗАПИСЕЙ
delete from '||load_||'.prig_lgot_stat where yymm in(select yymm from '||load_||'.prig_work where rez=11);


/*  --ВРЕМЕННО ОТМЕНЯЮ ЖУТКО МЕДЛЕННЫЙ БЛОК!
...
insert into '||load_||'.prig_work (yymm,fio,kodbl,rez,kol_bil)
with del as
(select yymm,list,kodbl,kol from
(select yymm,list,kodbl,count(*) as kol,sum(p27) as plata,sum(p28) as poteri,sum(p16) as kol_bil
from '||load_||'.prig_lgot_reestr where yymm in(select yymm from '||load_||'.prig_work where rez=11)
...
update '||load_||'.prig_lgot_reestr set deleted=''1''  where (yymm,kodbl,list) in (select yymm,kodbl,fio from '||load_||'.prig_work where rez=12);

*/

--ЗАПОЛНЕНИЕ контрольной таблицы
insert into '||load_||'.prig_lgot_stat(yymm,list,dor,kol_zap,kol_del,kol_raz,kol_abon,kol_ab_k,plata,poteri,kol_porc,date_zap)
with ish as
(select yymm,list,
...
--GET DIAGNOSTICS integer_var := ROW_COUNT;
RAISE INFO ' ШАГ 4. ЗАПИСАНО В БАЗУ РЕЕСТРА ЛЬГОТНИКОВ  % записей. За % дату', integer_var, date_zap;	

--- ввод времени окончания операции с итоговым числом записей								  
execute ' 

insert into  '||load_||'.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,libr,rezult)
select time,date,dann,''work 2_4_lgot'' as oper,
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2,date_zap,part_zap,shema,libr, '||integer_var||' as rezult
from 
(select date_zap,part_zap,dann,shema,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time 
	from  '||load_||'.prig_times  where dann=''prig'' and oper=''dannie'') as a;
...
--Запись признака прочтения
execute '
update '||load_||'.prig_times set oper=''read'',date=current_date,time=substr(cast(current_time as char(50)),1,12)
	where dann=''prig'' and oper=''dannie''  and dann=''prig'' and itog is null;
	
--Постановка новой даты для расчёта - БОЛЬШЕ НЕ НУЖНО!
--delete from '||load_||'.prig_times where dann=''prig'' and oper=''dannie'';

/* 
 --Постановка новой порции для расчёта
insert into  '||load_||'.prig_times(oper,date_zap,dann,shema,libr,part_zap,rezult,min_id,max_id)
select oper,date_zap,dann,shema,libr,part_zap,rezult,min_id,max_id from
(select ''dannie'' as oper,date_zap,dann,shema,libr,part_zap,rezult,min_id,max_id,row_number() over(order by part_zap) as nn
from '||load_||'.prig_times where dann=''prig'' and oper=''dann'') as a where nn=1;
*/
...
ALTER PROCEDURE svod.prig_obog_4(text) OWNER TO asul; --компилляция!

 --   call svod.prig_obog_4('l3_prig');  
