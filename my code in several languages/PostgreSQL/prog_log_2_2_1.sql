...
--   DROP PROCEDURE svod.prig_obog_21(text)


CREATE OR REPLACE PROCEDURE svod.prig_obog_21(load_ text)
...
select time,date,dann,libr,''work 2_21_beg'' as oper,
...
(select idnum,sto,stn,nom,reg,srasst,k_bil,d_plata,d_poteri,marshr,st1,st2,rst as rasst,sto_zone,stn_zone,sti,sti_zone,
 ...
 max(nom) over (partition by IDNUM) as max_nom
	from '||load_||'.prig_work where rez=5),
...
RAISE INFO 'ШАГ 2. Выбрано % записей. За % дату % порция', integer_var, date_zap, part_zap;
...
(select date_zap,part_zap,dann,libr,shema,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time
	from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'')
select time,date,dann,libr,''work 2_21_rez'' as oper,
...
ALTER PROCEDURE svod.prig_obog_21(text) OWNER TO asul; --компилляция!


--   call svod.prig_obog_21('l3_prig'); --непосредственно запуск процедуры
