...
where i<srok_bil
 /*((i<srok_bil and srok_mon=0)  or 
	...
	--   and (flg_rab_day='0' or (flg_rab_day='1' and weekd in (0,6)) or (flg_rab_day='2' and weekd not in (0,6))) -- временно отменил для проверки
...
update spb_prig.prig_times set oper='dann_read' where oper in('dannie');
...
insert into  spb_prig.prig_times(oper,date_zap,rezult)
select 'dannie' as oper,date_zap,rezult from spb_prig.prig_times where oper='dann' and date_zap in(
select min(date_zap) as date_zap from spb_prig.prig_times where oper='dann');


 
 

select date_zap,count(*) from spb_prig.prig_agr_pereg group by 1 order by date_zap desc ;
