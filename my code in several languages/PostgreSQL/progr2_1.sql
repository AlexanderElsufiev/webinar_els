...
---- строку l3_prig. или l3_mes. заменить на '||load_||'.
...
insert into  l3_prig.prig_times(dann,time,date,oper,min_id,max_id,libr,time2,date_zap,part_zap,shema)
with a as
(select dann,date_zap,part_zap,shema,min_id,max_id,libr,current_date as date,substr(cast(current_time as char(50)),1,12) as time 
 from  l3_prig.prig_times where dann='prig' and oper='dannie' --and shema in('mon_prig','day_prig') 
)
select dann,time,date,'work 2_1_beg' as oper,min_id,max_id,libr,
...
(select distinct date_zap,part_zap,min_id,max_id from  l3_prig.prig_times  where dann='prig' and oper='dannie' and substr(shema,5,4)='prig'
...
insert into  l3_prig.prig_times(time,date,dann,oper,time2,date_zap,part_zap,shema,rezult)
with a as
(select date_zap,part_zap,dann,shema,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time
 from  l3_prig.prig_times where dann='prig' and oper='dannie' --and substr(shema,5,4)='prig'
),
b as (select count(*) as rezult from l3_prig.prig_work)
select time,date,dann,'work 2_1_rez' as oper,
