...
--  DROP PROCEDURE svod.prig_obog_1(text)




CREATE OR REPLACE PROCEDURE svod.prig_obog_1(load_ text)
...
execute 'select date_zap,part_zap,shema,rezult,libr,cast(current_time as char(50)) from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'' '
		into date_zap,part_zap,shema,integer_var,libr,calc_time;
...
/*libr='RAWDL2M';
if substr(shema,1,3)='tst' then libr='RAWDL2';end if;
if substr(shema,1,3)='day' then libr='RAWDL2s';end if;*/

--RAISE INFO ' схема чтения %',  libr;		

--- ввод в лог времени начала операции
execute '
insert into  '||load_||'.prig_times(time,date,dann,oper,min_id,max_id,libr,time2,date_zap,part_zap,shema)
with a as
(select date_zap,part_zap,dann,shema,min_id,max_id,libr,current_date as date,substr(cast(current_time as char(50)),1,12) as time 
 from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'' --and shema in(''mon_prig'',''day_prig'')
 )
select time,date,dann,''work 2_1_beg'' as oper,min_id,max_id,libr,
...
insert into '||load_||'.prig_work(
	rez,nom_bil,nom_mar,nom_dat,YYMM,date_zap,part_zap,date_beg,date_end,date_pr,idnum,request_num,TERM_DOR,term_pos,term_trm,time_zap,server_reqnum,drac,
	...
	bag_vid,bag_ves,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,TRAIN_CATEGORY,TRAIN_NUM,mcd,employee_cat,prod,flg_sbor,flg_bil_sbor,
	marshr,st1,st2,rst,sto_zone,stn_zone,sti,sti_zone,dor,lin,
fio,fio_2,snils,ticket,kodbl,benefit_doc,benefit_podr,bilgroup --поля для реестра льгнотников
	)
/**/
...
(select distinct date_zap,part_zap,min_id,max_id from  '||load_||'.prig_times  where dann=''prig'' and oper=''dannie'' and substr(shema,5,4)=''prig''
...
IDNUM,
case when substr('''||shema||''',1,3)=''tst'' then request_num else 0 end as request_num, --для нетестовых номер запроса делаем =0			 
request_num as req_num,--только  для льготников - оставляется в любом случае
			-- row_number() over (order by idnum) as idd,
YYYYMM-200000 as yymm,REQUEST_DATE as date_zap,TERM_POS,TERM_DOR,term_trm,
			 --cast(request_time as char(8)) 
server_datetime as time_zap,
cast(server_reqnum as char(10)) as server_reqnum,
cast(server_stcode as char(10)) as drac,
...
cast(SALE_STATION as dec(7)) as stp,cast(REGION_CODE as dec(3)) as stp_reg,
cast(DEPARTURE_STATION as dec(7)) as sto,DEPARTURE_ZONE as sto_zone,
cast(ARRIVAL_STATION as dec(7)) as stn,ARRIVAL_ZONE as stn_zone,
cast(INTERMED_STATION as dec(7)) as sti,INTERMED_ZONE as sti_zone,
...
--ПОКУДА НЕ ПЕРЕДЕЛАЛИ БАЗУ - В СТАРОЙ БАЗЕ БЕРУ ПОЛНЫЙ КОД ЛЬГОТЫ, В НОВОЙ ТОЛЬКО КОД БЕЗ ГРУППЫ (НАЗЫВАЯ ЕГО КОДОМ), ПОТОМ БРАТЬ ОДИНАКОВО У ВСЕХ!!!
/*cast(BENEFITGROUP_CODE as dec(3))*100+*/cast(BENEFIT_CODE as dec(5)) as kod_lgt,
cast(BENEFIT_REGION as dec(3)) as lgt_reg,
...
case when ticket_ser is null then '''' else trim(substr(ticket_ser,1,2)) end || 
 case when ticket_num<1000000 then to_char(ticket_num,''fm000000'') else to_char(ticket_num,''fm0000000'') end as ticket,
 kodbl,

PASS_QTY as kol_bil, --в том числе и багажные билеты, по многу штук.
--деньги остаются в гривенниках, а не в копейках, без домножения на *10			 
TARIFF_SUM*1 as plata,
DEPARTMENT_SUM*1 as poteri,
(case when TOTAL_SUM=0 then 0
	...
	else TOTAL_SUM-TARIFF_SUM end)*1 as perebor,
fee_sum*1 as kom_sbor,0 as kom_sbor_vz,			 
...
from '||libr||'.l2_prig_main   where REQUEST_DATE  in (select date_zap from dates)
 and idnum between (select min_id from dates) and (select max_id from dates)
 and no_use is null
 ),
...
(select  IDNUM,doc_reg as nom,
--YYMM,REQUEST_DATE,REQUEST_NUM,TERM_POS,TERM_DOR,TERM_TRM,ARXIV_CODE,REPLY_CODE, --не нужны
	ROUTE_NUM as marshr,cast(ROUTE_DISTANCE as dec(7)) as rasst,
	sum(cast(ROUTE_DISTANCE as dec(7))) over(partition by IDNUM) as srasst,
	TARIFF_SUM as d_plata,DEPARTMENT_SUM as d_poteri,
	cast(DEPARTURE_STATION as dec(7)) as st1,cast(ARRIVAL_STATION as dec(7)) as st2,
 cast(REGION_CODE as dec(3)) as reg,
	max(doc_reg) over (partition by IDNUM) as max_nom, 
	sum(TARIFF_SUM) over (partition by IDNUM) as s_plata,
	sum(DEPARTMENT_SUM) over (partition by IDNUM) as s_poteri 
from '||libr||'.l2_prig_cost where REQUEST_DATE in(select date_zap from dates) and 
 	idnum in(select idnum from prig_main)
),

prig_fio as
(select IDNUM,substr(surname||'' ''||initials,1,45) as fio,
 ...
 /*	case when ticket_ser is null then ''--'' else substr(ticket_ser,1,2) end ||
		--перевод в строку с лидирующими нулями
		to_char(ticket_num,''fm0000000'') as ticket, */
	benefit_doc,employee_unit as benefit_podr,snils,
 case when employee_cat is null then ''-'' else employee_cat end as employee_cat
	from '||libr||'.l2_prig_adi where REQUEST_DATE in(select date_zap from dates) and 
 		idnum in(select idnum from prig_main)
),

prig_main_ as 
(select a.IDNUM,request_num,req_num,YYMM,date_zap,TERM_POS,TERM_DOR,term_trm,
...
bilgroup,employee_cat,fio,fio_2,ticket,kodbl,benefit_doc,benefit_podr,snils,
case when bilgroup is NULL then ''0'' else substr(bilgroup,3,1) end as rzd_fpk  
 from prig_main as a left join prig_fio as b on a.idnum=b.idnum),
 
 
	
prig_cost2 as --исходник пригород по перегонам по субъектам, с добавкой числа билетов
(select  a.IDNUM,nom,a.marshr,/*train_num,*/a.rasst,a.st1,a.st2,a.reg as regg,max_nom,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,
...
from prig_cost as a join prig_main as b on a.idnum=b.idnum
),
...
(select IDNUM,1 as k_bil from --определяем 1 как максимальный делитель для плохих. а хорошим оставляем реальные количества билетов
(select distinct IDNUM from prig_cost2 where mod(plata,kol_bil)!=0 or mod(poteri,kol_bil)!=0
...
prig_cost2_kbil as
(select IDNUM,nom,marshr,/*train_num,*/mcd,rasst,st1,st2,regg,max_nom,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,k_bil,
 	...
 	from
(select a.IDNUM,nom,marshr,/*train_num,*/mcd,rasst,st1,st2,regg,max_nom,kol_bil,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,
 	plata,poteri,d_plata,d_poteri,
 	case when k_bil is not null then k_bil else abs(kol_bil) end as k_bil 
 from prig_cost2 as a left join prig_cost2_kbil_nod as b on a.idnum=b.idnum
	) as c),
...
(select IDNUM,
 nom,marshr,/*train_num,*/mcd,rasst,st1,st2,regg,max_nom,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,k_bil,
...
(select IDNUM,nom,marshr,/*train_num,*/mcd,rasst,st1,st2,regg,max_nom,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,k_bil,
 ...
 min(case when mod(d_plata,64)=0 then 64 when mod(d_plata,32)=0 then 32 when mod(d_plata,16)=0 then 16
 	...
 	over (partition by IDNUM) as dpl2,
 min(case when mod(d_plata,729)=0 then 729 when mod(d_plata,243)=0 then 243 when mod(d_plata,81)=0 then 81
 	...
 	over (partition by IDNUM) as dpl3,
 min(case when mod(d_plata,125)=0 then 125 when mod(d_plata,25)=0 then 25 when mod(d_plata,5)=0 then 5 else 1 end) 
 	over (partition by IDNUM) as dpl5,
 min(case when mod(d_plata,49)=0 then 49 when mod(d_plata,7)=0 then 7 else 1 end) 
 	over (partition by IDNUM) as dpl7,
 min(case when mod(d_plata,11)=0 then 11 else 1 end) over (partition by IDNUM) as dpl11,
 min(case when mod(d_plata,13)=0 then 13 else 1 end) over (partition by IDNUM) as dpl13,
 min(case when mod(d_plata,17)=0 then 17 else 1 end) over (partition by IDNUM) as dpl17,
 ...
 min(case when mod(d_poteri,64)=0 then 64 when mod(d_poteri,32)=0 then 32 when mod(d_poteri,16)=0 then 16
 	...
 	over (partition by IDNUM) as dpot2,
 min(case when mod(d_poteri,729)=0 then 729 when mod(d_poteri,243)=0 then 243 when mod(d_poteri,81)=0 then 81
 	...
 	over (partition by IDNUM) as dpot3,
 min(case when mod(d_poteri,125)=0 then 125 when mod(d_poteri,25)=0 then 25 when mod(d_poteri,5)=0 then 5 else 1 end) 
 	over (partition by IDNUM) as dpot5,
 min(case when mod(d_poteri,49)=0 then 49 when mod(d_poteri,7)=0 then 7 else 1 end) 
 	over (partition by IDNUM) as dpot7, 
 min(case when mod(d_poteri,11)=0 then 11 else 1 end) over (partition by IDNUM) as dpot11,
 min(case when mod(d_poteri,13)=0 then 13 else 1 end) over (partition by IDNUM) as dpot13,
 min(case when mod(d_poteri,17)=0 then 17 else 1 end) over (partition by IDNUM) as dpot17
...
bil_ as --не просто distinct ДЛЯ ТОГО, ЧТОБЫ НАЙТИ представителя (IDNUM)
...
k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,idnum 
 from
(select flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,idnum,
 min(IDNUM) over(partition by flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor) as mIDNUM
 from prig_main_
) as a where IDNUM=mIDNUM),


bil as --добавляется значение поля flg_bil_sbor с раздвоеием записи - если запись типа Сбор в пути следования.  20/02/2023
(select  idnum,flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,''B'' as flg_bil_sbor
 from bil_
 union all
 select  idnum,flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,''S'' as flg_bil_sbor
 from bil_ where substr(flg_sbor,1,1)=''1''),

old_bil as --Сделать выборку из справочника, когда будет создан
(select * from '||load_||'.prig_bil),
...
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 ...
 ROW_NUMBER() over ( order by (nom_bil<0),nom_bil) as nom_bil,idnum
 from 
(select flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,flg_bil_sbor,
 max(nom_bil) as nom_bil,min(nom_bil) as min_bil,min(idnum) as idnum
 from 
(select flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,flg_bil_sbor,
 -1 as nom_bil,IDNUM from bil
union
select flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,flg_bil_sbor,
...
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
 k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,flg_bil_sbor) as b),
...
(select date_beg,return_date,k_pas,srok_bil,srok_mon,flg_rab_day,idnum,
 	ROW_NUMBER() over (order by date_beg,return_date,k_pas,srok_bil,srok_mon,flg_rab_day) as ndt  from
(select date_beg,return_date,k_pas,srok_bil,srok_mon,flg_rab_day,idnum,
 	min(IDNUM) over (partition by date_beg,return_date,k_pas,srok_bil,srok_mon,flg_rab_day) as mIDNUM
from prig_main) as a where IDNUM=mIDNUM),
...
(select nom_dat,0 as ndt,plus_dat as i,kpas_day from '||load_||'.prig_dats
union all
...
(select c.ndt,nom_dat,i,kpas_day,e.idnum from
...
(select idnum,nom_dat,c.date_beg,c.return_date,mplus_dat+c.date_beg as date_end,date_pr from prig_main as c join
...
prig_main_bil as
(select a.IDnum,request_num,YYMM,date_zap,nom_bil,TERM_DOR,term_pos,term_trm,time_zap,server_reqnum,drac,agent,chp,stp,stp_reg,train_num,
 ...
 a.TRAIN_CATEGORY=b.TRAIN_CATEGORY and a.employee_cat=b.employee_cat
...
--обогощение по датам
select 4 as rez,NULL as nom_bil,0 as nom_mar,nom_dat,0 as YYMM,/*NULL as date_zap,*/date_beg,date_end,date_pr,
 idnum,0 request_num,NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,NULL as server_reqnum,NULL as drac,
 ...
 NULL as TRAIN_CATEGORY,NULL as TRAIN_NUM,0 as mcd,NULL as employee_cat,NULL as prod,NULL as flg_sbor,NULL as flg_bil_sbor,
 ...
 NULL as fio,NULL as fio_2,NULL as snils,NULL as ticket,NULL as kodbl,NULL as benefit_doc,NULL as benefit_podr,NULL as bilgroup 
 from dats_itog2

union all 
select   --расписывание уникальных дат  
 1 as rez,0 as nom_bil,0 as nom_mar,nom_dat,0 as YYMM,/*date_zap,*/ NULL as date_beg,NULL as date_end,NULL as date_pr,idnum,0 as request_num,
...
select 2 as rez,nom_bil,0 as nom_mar,NULL as nom_dat,0 as YYMM,/*date_zap,*/NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,0 as request_num,NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,NULL as server_reqnum,NULL as drac,
 0 as agent,0 as chp,0 as stp,0 as stp_reg,0 as sto,0 as stn,0 as sto_reg,0 as stn_reg,0 as srasst,
...
klass,kod_lgt,lgt_reg,bag_vid,bag_ves,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,TRAIN_CATEGORY,NULL as TRAIN_NUM,0 as mcd,employee_cat,prod,flg_sbor,flg_bil_sbor,
 0 as marshr,NULL as st1,NULL as st2,NULL as rst,NULL as sto_zone,NULL as stn_zone,NULL as sti,NULL as sti_zone,
 NULL as dor,NULL as lin,
 NULL as fio,NULL as fio_2,NULL as snils,NULL as ticket,NULL as kodbl,NULL as benefit_doc,NULL as benefit_podr,NULL as bilgroup 
 from new_bil_zapis
...
select 3 as rez,nom_bil,0 as nom_mar,NULL as nom_dat,YYMM,/*date_zap,*/NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,request_num,TERM_DOR,term_pos,term_trm,time_zap,server_reqnum,drac,
 ...
 NULL as FLG_SO,NULL as FLG_NU,NULL as FLG_TT,NULL as klass,NULL as kod_lgt,NULL as lgt_reg,NULL as bag_vid,NULL as bag_ves,
 NULL as proc_lgt,NULL as rzd_fpk,NULL as ABONEMENT_TYPE,NULL as ABONEMENT_SUBTYPE,NULL as FLG_OFFICIAL_BENEFIT,
 NULL as TRAIN_CATEGORY,TRAIN_NUM,0 as mcd,NULL as employee_cat,NULL as prod,NULL as flg_sbor,NULL as flg_bil_sbor,
 0 as marshr,NULL as st1,NULL as st2,NULL as rst,NULL as sto_zone,NULL as stn_zone,NULL as sti,NULL as sti_zone,
 NULL as dor,NULL as lin,
 NULL as fio,NULL as fio_2,NULL as snils,NULL as ticket,NULL as kodbl,NULL as benefit_doc,NULL as benefit_podr,NULL as bilgroup 
 from prig_main_bil
...
select 5 as rez,NULL as nom_bil,0 as nom_mar,NULL as nom_dat,0 as YYMM,/*NULL as date_zap,*/NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,0 as request_num,NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,NULL as server_reqnum,NULL as drac,
 ...
 NULL as FLG_SO,NULL as FLG_NU,NULL as FLG_TT,NULL as klass,NULL as kod_lgt,NULL as lgt_reg,NULL as bag_vid,NULL as bag_ves,
 NULL as proc_lgt,NULL as rzd_fpk,NULL as ABONEMENT_TYPE,NULL as ABONEMENT_SUBTYPE,NULL as FLG_OFFICIAL_BENEFIT,
 NULL as TRAIN_CATEGORY,NULL as TRAIN_NUM,mcd,NULL as employee_cat,NULL as prod,NULL as flg_sbor,NULL as flg_bil_sbor,
...
select   --расписывание фамилии для реестра
 0 as rez,0 as nom_bil,0 as nom_mar,0 as nom_dat,0 as YYMM,/*date_zap,*/ NULL as date_beg,NULL as date_end,NULL as date_pr,idnum,req_num as request_num,
 NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,NULL as server_reqnum,NULL as drac,0 as agent,0 as chp,0 as stp,0 as stp_reg,
 ...
 0 as marshr,0 as st1,0 as st2,0 as rst,0 as sto_zone,0 as stn_zone,0 as sti,0 as sti_zone,
0 as dor,0 as lin,
fio,fio_2,snils,ticket,kodbl,benefit_doc,benefit_podr,bilgroup 
...
/**/
select rez,nom_bil,nom_mar,nom_dat,YYMM,date_zap,part_zap,date_beg,date_end,date_pr,idnum,request_num,TERM_DOR,term_pos,term_trm,time_zap,server_reqnum,drac,
...
RAISE INFO 'ШАГ 1. Выбрано % записей. За % дату % порция', integer_var, date_zap, part_zap;	
...
execute ' 
insert into  '||load_||'.prig_times(time,date,dann,oper,min_id,max_id,libr,time2,date_zap,part_zap,shema,rezult)
with a as
(select date_zap,part_zap,dann,shema,min_id,max_id,libr,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time
 from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'' --and substr(shema,5,4)=''prig''
 )
select time,date,dann,''work 2_1_rez'' as oper,min_id,max_id,libr,
...
ALTER PROCEDURE svod.prig_obog_1(text) OWNER TO asul; --компилляция!


--    call svod.prig_obog_1('l3_prig');    --непосредственно запуск процедуры
