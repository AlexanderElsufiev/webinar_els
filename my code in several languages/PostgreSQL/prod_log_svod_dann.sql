...
CREATE OR REPLACE PROCEDURE svod.svod_dann(load_ text)
...
insert into  '||load_||'.prig_times(dann,oper,date_zap,rezult,min_id,max_id,shema,libr,part_zap)
with
...
insert into  '||load_||'.prig_times(dann,oper,date_zap,shema,libr,part_zap,rezult,min_id,max_id,min_id_svod)
with times as
...
ALTER PROCEDURE svod.svod_dann(text) OWNER TO asul; --компилляция!


--    call svod.svod_dann('l3_mes');    --непосредственно запуск процедуры
