...
CREATE OR REPLACE PROCEDURE svod.prig_agreg_1(load_schema text)
...
execute 'delete from l3_prig.prig_times where oper=''to_agregate_kst'';' ;
...
execute 'select date_zap,part_zap from  l3_prig.prig_times where oper=''to_agregate_kst'' '
		into calc_date,calc_part;

RAISE INFO ' Начата агрегация постанционная За % дату % порция',  calc_date, calc_part;
...
RAISE INFO 'Закончена агрегация постанционная За % дату % порция, итог = % записей',  calc_date, calc_part, integer_var;
...
(cast(substr(time,1,2) as dec(3))*60+cast(substr(time,4,2) as dec(3)))*60+cast(substr(time,7,6) as dec(7,3)) as time2
from
(select date_zap,part_zap,shema,'||integer_var||' as rezult,''write_agr_kst'' as oper,
 current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time
 from dat as b) as c;
...
ALTER PROCEDURE svod.prig_agreg_1(text) OWNER TO asul; --компилляция!


--   call svod.prig_agreg_1('1'); --непосредственно запуск процедуры
