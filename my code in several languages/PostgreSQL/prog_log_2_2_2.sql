...
--  DROP PROCEDURE svod.prig_obog_22(text)


CREATE OR REPLACE PROCEDURE svod.prig_obog_22(load_ text)
...
select time,date,dann,libr,''work 2_22_beg'' as oper,
...
(select  IDNUM,opis,nom,marshr,rst as rasst,st1,st2,reg as regg,/*max_nom,*/sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,d_plata,d_poteri,mcd
 from '||load_||'.prig_work where rez=51),
...
RAISE INFO 'ШАГ 2_2. Выбрано % записей. За % дату % порция', integer_var, date_zap, part_zap;
...
(select date_zap,part_zap,dann,libr,shema,current_date as date,substr(cast(clock_timestamp()::time as char(50)),1,12) as time
	from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'')
select time,date,dann,libr,''work 2_22_rez'' as oper,
...
--return 'Success prig_obog_2';
end;
$BODY$;


ALTER PROCEDURE svod.prig_obog_22(text) OWNER TO asul; --компилляция!


--   call svod.prig_obog_22('l3_prig'); --непосредственно запуск процедуры
