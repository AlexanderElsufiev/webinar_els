...
----- from rawdl2.    from '||libr||'.
---- строку 'tst_prig' заменить на '||shema||'  в итоге получается  '''||shema||'''
---- строку 'l3_prig' заменить на '||load_||'

--  DROP PROCEDURE svod.prig_obog_1p(text)


CREATE OR REPLACE PROCEDURE svod.prig_obog_1p(load_ text)
...
execute 'select date_zap,part_zap,shema,libr from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'' '
		into date_zap,part_zap,shema,libr;
		
if substr(shema,5,4)='pass' then begin
...
execute 'delete from '||load_||'.prig_work;' ;

--удаление старья из таблицы логов за рабочую дату
execute '
delete from '||load_||'.prig_times where part_zap in(select part_zap from '||load_||'.prig_times where oper=''dannie'' --and substr(shema,5,4)=''pass''
)
...
(select date_zap,part_zap,dann,libr,shema,current_date as date,substr(cast(current_time as char(50)),1,12) as time 
 from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie''  and substr(shema,5,4)=''pass''
 )
select time,date,dann,libr,''work 2_1p_beg'' as oper,
...
RAISE INFO ' Начата первая часть обогащения За % дату % порция, пассажиры дальние',  date_zap, part_zap;
...
insert into '||load_||'.prig_work(
	rez,nom_bil,nom_mar,nom_dat,YYMM,date_zap,part_zap,date_beg,date_end,date_pr,idnum,request_num,TERM_DOR,term_pos,term_trm,time_zap,
	...
	plata,poteri,perebor,kom_sbor,kom_sbor_vz,ticket,kodbl,snils,fio,fio_2,bilgroup,benefit_doc,nom,reg,d_plata,d_poteri,
	...
	TRAIN_CATEGORY,TRAIN_NUM,mcd,employee_cat,prod,flg_sbor,flg_bil_sbor,marshr,st1,st2,rst,sto_zone,stn_zone,sti,sti_zone,dor,lin )
/**/
...
(select distinct date_zap,part_zap,min_id,max_id from  '||load_||'.prig_times where dann=''prig'' and oper=''dannie'' and substr(shema,5,4)=''pass''),

prig_main as --(select * from 
(select
...
kodbl,snils,YYYYMM-200000 as yymm,REQUEST_DATE as date_zap,TERM_POS,TERM_DOR,term_trm,
''0'' as time_zap,
OPER_DATE as date_pr,DEPARTURE_DATE as date_beg,''1'' AS TRAIN_CATEGORY,TRAIN_NUM,
...
cast(SALE_STATION as dec(7)) as stp,cast(saleREGION_CODE as dec(3)) as stp_reg,
cast(DEPARTURE_STATION as dec(7)) as sto,/*DEPARTURE_ZONE*/ 99 as sto_zone,
cast(ARRIVAL_STATION as dec(7)) as stn,/*ARRIVAL_ZONE*/99 as stn_zone,
...
cast(BENEFIT_CODE as dec(5)) as kod_lgt,
...
from '||libr||'.l2_pass_main   where REQUEST_DATE  in (select date_zap from dates) and
	...
	--and id=457321129	
 --) as a,(select min_id,max_id from dates) as b where id between min_id and max_id 
...
(select  IDNUM,ticket_ser,ticket_num,
min(case when PAYMENTTYPE in (''1'', ''6'' ,''8'') then paymenttype else ''9'' end) vid_rasch2,
--все деньги домножаются на 10, а не на 100, до целых гривенников а не копеек.
sum(case when sum_code in(104,105) then 0 when PAYMENTTYPE in (''1'', ''6'' ,''8'') then SUM_NDE else 0 end)*10 as plata, ---PAYMENTTYPE = вида расчета
sum(case when sum_code in(104,105) then 0 when PAYMENTTYPE in (''1'', ''6'' ,''8'') then 0 else SUM_nde end)*10 as poteri, ---PAYMENTTYPE = вида расчета
sum(case when sum_code in(104) then sum_nde else 0 end)*10 as kom_sbor,
sum(case when sum_code in(105) then sum_nde else 0 end)*10 as kom_sbor_vz
   
from '||libr||'.l2_pass_cost where REQUEST_DATE in(select date_zap from dates) and
  idnum in (select idnum from prig_main)
 group by idnum,ticket_ser,ticket_num
),
...
from 
(select IDNUM,ticket_ser,ticket_num,npp,
 substr(last_name||'' ''||
...
(case when first_name is NULL then patronymic else substr(patronymic,1,1) end),1,45) as fio, 
 ...
 nomlgud as benefit_doc
	from '||libr||'.l2_pass_ex where REQUEST_DATE in(select date_zap from dates) and
  idnum in (select idnum from prig_main)
) as a  group by 1,2,3),


prig_main2 as
(select a.IDNUM,request_num,req_num,
YYMM,date_zap,TERM_POS,TERM_DOR,term_trm,time_zap,date_pr,date_beg,TRAIN_CATEGORY,TRAIN_NUM,employee_cat,prod,flg_sbor,--koef,
...
FLG_tuda_obr,k_tuda_obr,kol_bil,plata,poteri,perebor,kom_sbor,kom_sbor_vz,ticket,kodbl,snils,fio,fio_2,benefit_doc,proc_lgt,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
...
from prig_main as a join prig_cena as b on a.idnum=b.idnum and a.ticket_ser=b.ticket_ser and a.ticket_num=b.ticket_num
 left join lgot as c on kod_lgt=lgot			  
 left join pass_fio d on a.idnum=d.idnum and a.ticket_ser=d.ticket_ser and a.ticket_num=d.ticket_num
			),

/*
prig_cost as --исходник пригород по перегонам по субъектам
(select  IDNUM,1 as nom,--1 as mnom, 
...
(select  IDNUM,1 as nom,0 as marshr,rasst,sto as st1,stn as st2,0 as regg,1 as max_nom,sto,stn,rasst as srasst,sto_zone,stn_zone,sti,sti_zone,
...
from /*prig_cost as a join*/ prig_main2 /*as b on a.idnum=b.idnum*/
),
...
prig_cost2_kbil as
(select IDNUM,nom,marshr,rasst,st1,st2,regg,max_nom,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,k_bil,
 	...
 	from
(select a.IDNUM,nom,marshr,rasst,st1,st2,regg,max_nom,kol_bil,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,
...
(select IDNUM,
 nom,marshr,rasst,st1,st2,regg,max_nom,sto,stn,srasst,sto_zone,stn_zone,sti,sti_zone,k_bil,
...
bil as --не просто distinct ДЛЯ ТОГО, ЧТОБЫ НАЙТИ IDNUM
...
k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,''B'' as flg_bil_sbor,idnum from
(select flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
klass,kod_lgt,lgt_reg,flg_bag,bag_vid,bag_ves,FLG_tuda_obr,flg_rab_day,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,
k_pas,srok_bil,srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor,
idnum,min(idnum) over(partition by flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,FLG_BSP,FLG_SO,FLG_NU,FLG_TT,
...
srok_mon,oper,oper_g,TRAIN_CATEGORY,employee_cat,prod,flg_sbor) as midnum
 from prig_main2) as a where idnum=midnum),
...
(select date_beg,k_pas,srok_bil,srok_mon,flg_rab_day,idnum,
 	ROW_NUMBER() over (order by date_beg,k_pas,srok_bil,srok_mon,flg_rab_day) as ndt  from
(select date_beg,k_pas,srok_bil,srok_mon,flg_rab_day,idnum,
 	min(idnum) over (partition by date_beg,k_pas,srok_bil,srok_mon,flg_rab_day) as midnum
from prig_main2) as a where idnum=midnum),
...
(select idnum,nom_dat,c.date_beg,mplus_dat+c.date_beg as date_end,date_pr from prig_main2 as c join
...
prig_main_bil as
(select a.IDNUM,request_num,req_num,YYMM,date_zap,nom_bil,TERM_DOR,term_pos,term_trm,time_zap,agent,subagent,chp,stp,stp_reg,train_num,
 ...
 ticket,kodbl,snils,fio,fio_2,bilgroup,benefit_doc,a.kod_lgt
 ...
 a.TRAIN_CATEGORY=b.TRAIN_CATEGORY and a.prod=b.prod and a.flg_sbor=b.flg_sbor and a.flg_rab_day=b.flg_rab_day
),
...
(
 ...
 --обогощение по датам
select 4 as rez,NULL as nom_bil,0 as nom_mar,nom_dat,0 as YYMM,date_beg,date_end,date_pr,
 idnum,0 as request_num,NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,
 ...
 0 as plata,0 as poteri,0 as perebor,0 as kom_sbor,0 as kom_sbor_vz,NULL as ticket,NULL as kodbl,NULL as snils,NULL as fio,NULL as fio_2,NULL as bilgroup,NULL as benefit_doc,0 as nom,0 as reg,0 as d_plata,0 as d_poteri,
...
select   --расписывание уникальных дат  
 1 as rez,0 as nom_bil,0 as nom_mar,nom_dat,0 as YYMM,NULL as date_beg,NULL as date_end,NULL as date_pr,idnum,0 as request_num,
 ...
 0 as plata,0 as poteri,0 as perebor,0 as kom_sbor,0 as kom_sbor_vz,NULL as ticket,NULL as kodbl,NULL as snils,NULL as fio,NULL as fio_2,NULL as bilgroup,NULL as benefit_doc,0 as nom,0 as reg,
...
select 2 as rez,nom_bil,0 as nom_mar,NULL as nom_dat,0 as YYMM,NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,0 as request_num,NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,
 0 as agent,0 as subagent,0 as chp,0 as stp,0 as stp_reg,0 as sto,0 as stn,0 as sto_reg,0 as stn_reg,0 as srasst,
 ...
 NULL as ticket,NULL as kodbl,NULL as snils,NULL as fio,NULL as fio_2,NULL as bilgroup,NULL as benefit_doc,0 as nom,0 as reg,0 as d_plata,0 as d_poteri,
...
klass,kod_lgt,lgt_reg,bag_vid,bag_ves,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,TRAIN_CATEGORY,NULL as TRAIN_NUM,employee_cat,prod,flg_sbor,flg_bil_sbor,
 0 as marshr,NULL as st1,NULL as st2,NULL as rst,NULL as sto_zone,NULL as stn_zone,NULL as sti,NULL as sti_zone,
 NULL as dor,NULL as lin
 from new_bil_zapis
...
select 3 as rez,nom_bil,0 as nom_mar,NULL as nom_dat,YYMM,NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,request_num,TERM_DOR,term_pos,term_trm,time_zap,
 ...
 plata,poteri,perebor,kom_sbor,kom_sbor_vz,NULL as ticket,NULL as kodbl,NULL as snils,NULL as fio,NULL as fio_2,NULL as bilgroup,NULL as benefit_doc,0 as nom,0 as reg,0 as d_plata,0 as d_poteri,
 NULL as flg_ruch,0 as request_type,0 as request_subtype,NULL as web_id,NULL as vid_rasch,NULL as FLG_CHILD,NULL as flg_voin,NULL as FLG_MILITARY,NULL as flg_lgt,NULL as FLG_BSP,
 NULL as FLG_SO,NULL as FLG_NU,NULL as FLG_TT,NULL as klass,0 as kod_lgt,NULL as lgt_reg,NULL as bag_vid,NULL as bag_ves,
 NULL as proc_lgt,NULL as rzd_fpk,NULL as ABONEMENT_TYPE,NULL as ABONEMENT_SUBTYPE,NULL as FLG_OFFICIAL_BENEFIT,
...
select 5 as rez,NULL as nom_bil,0 as nom_mar,NULL as nom_dat,0 as YYMM,NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,0 as request_num,NULL as TERM_DOR,NULL as term_pos,NULL as term_trm,NULL as time_zap,
 ...
 0 as plata,0 as poteri,0 as perebor,0 as kom_sbor,0 as kom_sbor_vz,NULL as ticket,NULL as kodbl,NULL as snils,NULL as fio,NULL as fio_2,NULL as bilgroup,NULL as benefit_doc,nom,regg as reg,d_plata,d_poteri,
 NULL as flg_ruch,0 as request_type,0 as request_subtype,NULL as web_id,NULL as vid_rasch,NULL as FLG_CHILD,NULL as flg_voin,NULL as FLG_MILITARY,NULL as flg_lgt,NULL as FLG_BSP,
 NULL as FLG_SO,NULL as FLG_NU,NULL as FLG_TT,NULL as klass,0 as kod_lgt,NULL as lgt_reg,NULL as bag_vid,NULL as bag_ves,
 NULL as proc_lgt,NULL as rzd_fpk,NULL as ABONEMENT_TYPE,NULL as ABONEMENT_SUBTYPE,NULL as FLG_OFFICIAL_BENEFIT,
 NULL as TRAIN_CATEGORY,NULL as TRAIN_NUM,NULL as employee_cat,NULL as prod,NULL as flg_sbor,NULL as flg_bil_sbor,
...
select 0 as rez,nom_bil,0 as nom_mar,NULL as nom_dat,YYMM,NULL as date_beg,NULL as date_end,NULL as date_pr,
 idnum,req_num as request_num,TERM_DOR,term_pos,term_trm,time_zap,
 0 as agent,0 as subagent,0 as chp,0 as stp,0 as stp_reg,0 as sto,0 as stn,0 as sto_reg,0 as stn_reg,0 as srasst,
 NULL as k_pas,NULL as srok_bil,NULL as srok_mon,NULL as oper,NULL as oper_g,NULL as flg_bag,NULL as FLG_tuda_obr,NULL as flg_rab_day,kol_bil,0 as k_bil,
 0 as plata,0 as poteri,0 as perebor,0 as kom_sbor,0 as kom_sbor_vz,ticket,kodbl,snils,fio,fio_2,bilgroup,benefit_doc,0 as nom,0 as reg,0 as d_plata,0 as d_poteri,
 NULL as flg_ruch,0 as request_type,0 as request_subtype,NULL as web_id,NULL as vid_rasch,NULL as FLG_CHILD,NULL as flg_voin,NULL as FLG_MILITARY,NULL as flg_lgt,NULL as FLG_BSP,
 NULL as FLG_SO,NULL as FLG_NU,NULL as FLG_TT,NULL as klass,0 as kod_lgt,NULL as lgt_reg,NULL as bag_vid,NULL as bag_ves,
 NULL as proc_lgt,NULL as rzd_fpk,NULL as ABONEMENT_TYPE,NULL as ABONEMENT_SUBTYPE,NULL as FLG_OFFICIAL_BENEFIT,
 NULL as TRAIN_CATEGORY,NULL as TRAIN_NUM,NULL as employee_cat,NULL as prod,NULL as flg_sbor,NULL as flg_bil_sbor,
...
/**/
select rez,nom_bil,nom_mar,nom_dat,YYMM,date_zap,part_zap,date_beg,date_end,date_pr,idnum,request_num,TERM_DOR,term_pos,term_trm,time_zap,
	agent,subagent,chp,stp,stp_reg,sto,stn,sto_reg,stn_reg,srasst,k_pas,srok_bil,srok_mon,oper,oper_g,flg_bag,FLG_tuda_obr,flg_rab_day,kol_bil,k_bil,
	plata,poteri,perebor,kom_sbor,kom_sbor_vz,ticket,kodbl,snils,fio,fio_2,bilgroup,benefit_doc,nom,reg,d_plata,d_poteri,
	flg_ruch,request_type,request_subtype,web_id,vid_rasch,FLG_CHILD,flg_voin,FLG_MILITARY,flg_lgt,
	FLG_BSP,FLG_SO,FLG_NU,FLG_TT,klass,kod_lgt,lgt_reg,
	bag_vid,bag_ves,proc_lgt,rzd_fpk,ABONEMENT_TYPE,ABONEMENT_SUBTYPE,FLG_OFFICIAL_BENEFIT,TRAIN_CATEGORY,TRAIN_NUM,0 as mcd,employee_cat,prod,flg_sbor,flg_bil_sbor,
...
RAISE INFO 'ШАГ 1. Выбрано % записей. За % дату % порция, пассажиры дальние', integer_var, date_zap, part_zap;	
...
select time,date,dann,libr,''work 2_1p_rez'' as oper,
...
--return 'Success prig_obog_1p';

end;
$BODY$;


ALTER PROCEDURE svod.prig_obog_1p(text) OWNER TO asul; --компилляция!


-- call svod.prig_obog_1p('l3_prig'); --непосредственно запуск процедуры
